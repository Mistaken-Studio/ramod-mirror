...
namespace Mistaken.RAMod
{
    public static class PlayerInfoHandler
    {
        ...
        public static readonly Dictionary<string, Func<Player, bool, string>> NormalParsers = new Dictionary<string, Func<Player, bool, string>>
        {
            ...
            {
                "nickname",
                (player, gameplayData) => player.TryGetSessionVariable("REAL_NICKNAME", out string nickname) ? $"{player.ReferenceHub.nicknameSync.CombinedName}* ({nickname})" : player.ReferenceHub.nicknameSync.CombinedName
            },
            {
                "playerId",
